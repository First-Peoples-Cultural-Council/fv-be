...
class BaseImportUpdateEndpoints(
    AsyncWorkflowTestMixin, FormDataMixin, BaseAsyncSiteContentApiTest
):
    """
    Shared API contract for import and update job endpoints.
    """
    ...
    @pytest.mark.parametrize(
        "job_status",
        [ImportJobStatus.ACCEPTED, ImportJobStatus.COMPLETE, ImportJobStatus.STARTED],
    )
    ...
    @pytest.mark.parametrize(
        "job_status", [ImportJobStatus.ACCEPTED, ImportJobStatus.STARTED]
    )
