...
class TestPermissionManager:
    @pytest.mark.parametrize(
        "model_info",
        [(DictionaryEntry, DictionaryEntryFactory)],
        ids=["DictionaryEntry"],
    )
    ...
    def assert_visible_matches(self, model_info, user, user_role):
        ...
        visible_by_permission_rules = self.get_permitted_ids(user, model.objects.all())
    ...
    def assert_visible_by_site(self, model_info, user, user_role):
        ...
        for s in sites:
            visible_by_permission_rules = self.get_permitted_ids(
    ...
    def get_permitted_ids(self, user, queryset):
