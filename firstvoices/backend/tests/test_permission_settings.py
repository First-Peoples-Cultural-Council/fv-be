...
class TestPermissionManager:
    models = apps.get_app_config("backend").get_models()
    ...
    def assert_visible_matches(self, model_cls, user, user_role):
        ...
        # generate test data with the necessary attributes
        if hasattr(model_cls, "site"):
            if hasattr(model_cls, "visibility"):
                self.generate_sites_with_controlled_test_data(
    ...
    def generate_sites_with_controlled_test_data(self, model_factory, user, user_role):
        for site_visibility in Visibility:
            # create a site with a membership
            site1 = factories.SiteFactory.create(visibility=site_visibility)
            factories.MembershipFactory(site=site1, user=user, role=user_role)
