...
def notify_job_ready(
    ...
    curr_job = job_model.objects.get(id=job_id)
    if curr_job.status == job_status_model.READY_FOR_IMPORT:
        raise ValidationError(already_ready_message)

    if curr_job.validation_status != job_status_model.COMPLETE:
    ...
    try:
        ...
        updated_job = job_model.objects.get(id=job_id)
        updated_job.status = job_status_model.READY_FOR_IMPORT
        updated_job.save()
