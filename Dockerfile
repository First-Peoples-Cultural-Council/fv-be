...
FROM --platform=linux/arm64/v8 python:3.11.4-alpine AS static-collector
RUN ["python3", "manage.py", "collectstatic"]

# select with --target static-runtime at build time
FROM --platform=linux/arm64/v8 $caddy_image AS static-runtime
...
FROM --platform=linux/arm64/v8 python:3.11.4-alpine AS build
EXPOSE 8000
